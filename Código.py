...
table = pd.read_excel("D:/Sebastian/Genetic-Algorithm/DATOS.xlsx")
# table = pd.read_excel("c:/Users/sophi/Desktop/U/Auxiliatura/DATOS.xlsx") ## Descomentar y comentar la linea de arriba en el pc de Sophía
table.head(11)
dist_d = []
...
for j in range(19):
    for i in range(len(table)):
        ...
        if np.random.uniform(0, 1) < table['Probriesgo1'].values[i]:
            dist_r1 = (
                stats.uniform(table['Riesgo1duracionmed'].values[i], table['Riesgo1duraciondesv'].values[i])).rvs(1)
        if np.random.uniform(0, 1) < table['Probriesgo2'].values[i]:
            dist_r2 = (
                stats.uniform(table['Riesgo2duracionmed'].values[i], table['Riesgo2duraciondesv'].values[i])).rvs(1)
        ...
        while len(prog) > 0:
            ...
            for _, p in enumerate(pol):
                if p in eleg and rec_1 >= recursos_1[p] and rec_2 >= recursos_2[p]:
                    prog.append(p)
                    tiempos.append(t_actual + durs[p])
                    rec_1 -= recursos_1[p]
                    rec_2 -= recursos_2[p]
            print( f"Actividades hechas: {done} Activdades programadas: {prog} Tiempo final: {t_actual} Recursos [{rec_1},{rec_2}]")
        Tfinales[i, j] = t_actual

        #     break
    # break

print(f"Promedios tiempos finales: {Tfinales.mean(axis=0)}")
